...
public:
	PicoSplitter( XmlConfig * config, string np, string fileList ="", string jobPrefix = "" ){
	    ...
	    logger->info( __FUNCTION__ ) << " First Tray : " << cfg->getInt( "Trays:min", 1 ) << endl;
	    int firstIndex = index( cfg->getInt( "Trays:min", 1 ) );
	    logger->info( __FUNCTION__ ) << "First Index " << firstIndex << endl;
	    ...
	    for ( int iTree = 0; iTree < nTrees; iTree++ ){
	    	...
	    	files.push_back( (new TFile(( cfg->getString( "output:path", "" ) +  "tree_" + ts( firstIndex + iTree ) + ".root").c_str(), "RECREATE") ) );
	...
	int index( int tray, int module = 1, int cell = 1 ){
	...
	int rIndex( int tray, int module = 1, int cell = 1 ){
	...
	void make(){
		...
		for(Int_t i=0; i<nEventsToProcess; i++) {
	    	...
	    	for ( int iHit = 0; iHit < nTofHits; iHit++ ){
	    		...
	    		int id = rIndex( tray, module, cell );
	    		
	    		if ( id >= 0  )
	    			logger->debug(__FUNCTION__) << "id " << id <<  " ( " << tray << ", " << module << ", " << cell << " ) " << endl;
	    		if ( nHits[ id ] >= kMaxHits || id < 0 || id > nTrees)
	    			continue;
